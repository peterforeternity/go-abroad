...
| Curated official-page crawler | Graduate programmes, admissions guidance, scholarships, tuition support and student-permit guidance | Fixed allowlist of government and university pages. The crawler stores an extract and always links to the original page; it does not claim the extract is complete. |
...
The curated crawler provides limited `major` and `scholarship` records from official pages. These are source-page summaries, not a comprehensive catalogue. No result is described as a comprehensive ranking or authoritative application decision. See `docs/crawler.md` for the allowlist, crawl controls, and failure semantics.
