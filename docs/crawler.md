...
- The seed count is capped by `CRAWLER_MAX_PAGES`; staging currently reviews 50 university admissions pages plus seven government/open-information pages.
- Fetches run in bounded batches of four so a slow university site cannot create unbounded concurrency.
...
Crawler output is stored in the staging KV namespace under `study-crawler:curated:v3`. The version advances when the reviewed source catalogue materially changes so the next scheduled run creates a fresh snapshot. The HTTP API only reads this snapshot. Network crawling happens through the scheduled synchronization path, so a user request never initiates a crawl.
...
| `CRAWLER_MAX_PAGES` | `57` | Hard cap covering 50 university pages and seven public-information pages. |
